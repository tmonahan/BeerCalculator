...
class RecipeStats extends Activity {
  ...
  private def getValue(name: String, scale: Float, offset: Int, defaultValue: Int): Int = {
    try {
      ((currentStyle \name).text.toFloat * scale).round - offset
    } catch {
  ...
  private def updateGravity() = {
    ...
    val sugar_kg = Calculation.getSugarFromFermentables(currentFermentables)
    val degrees_plato = Calculation.getDegreesPlato(sugar_kg, liters)
    val sg = Calculation.getSGfromPlato(degrees_plato)
  ...
  private def addFermentableToTable(v: View, node: FermentableSpinnerNode) = {
	  ...
	  tr.addView(deleteButton("Delete",() => {currentFermentables = removeNode(currentFermentables, node.node)} ))
  ...
  private def addHopsToTable(v: View, node: HopsSpinnerNode) = {
    ...
    tr.addView(timeText)
    tr.addView(amountText)
    tr.addView(deleteButton("Delete",() => {currentHops = removeNode(currentHops, node.node)} ))	  
      ...
      case FERMENTABLE_DIALOG => {
        ...
        addButton.setOnClickListener((v: View) => {
          ...
          if(amountTextView.getText().toString != ""){
	          ...
	          val node: NodeSeq = <FERMENTABLE>{ (fermentableContents \ "_").foldLeft(NodeSeq.Empty)((B: NodeSeq, myNode: Node) => {
	            myNode match {
      ...
      case HOPS_DIALOG => {
        ...
        addButton.setOnClickListener((v: View) =>{
          ...
          if(amountNumberPicker.getText().toString != "" && minutesNumberPicker.getText().toString != ""){
	          ...
	          val node: NodeSeq = <HOP>{ (hopContents \ "_").foldLeft(NodeSeq.Empty)((B: NodeSeq, myNode: Node) => {
	            myNode match {
